#!/usr/bin/env python3
"""
Crypto Macro Flow Dashboard - Single File Version
...
class AxeListGenerator:
    def __init__(self, config: Optional[Dict] = None):
        ...
        self.config = {
            'api_delay': 0.2, 'max_retries': 3, 'default_top_n': 50, 'progress_interval': 10
        }
        if config:
    ...
    def determine_baseline_asset(self) -> str:
        try:
            ...
            return 'BTC' if btc_outperforming else 'ETH'
    ...
    def run_analysis(self, top_n=50):
        ...
        st.info(f"🎯 Baseline Asset Determined: **{baseline}** (based on BTC/ETH 50-day MA analysis)")
    ...
    if page == "Current Quadrant Analysis":
        ...
        if daily_results is not None:
            ...
            with col2:
                st.metric("Primary Score", f"{current_score:.2f}", 
                         delta=f"{current_score - last_30_days['Primary_Score'].iloc[-2]:.2f}")
            
            with col3:
                st.metric("Confidence", 
                         "Very High" if np.isinf(current_data['Confidence']) else f"{current_data['Confidence']:.2f}")
        ...
        if daily_results is not None:
            ...
            with col2:
                if 'axe_data' in st.session_state and not st.session_state['axe_data'].empty:
                    axe_data = st.session_state['axe_data']
                    fig_top_tokens = px.bar(axe_data.head(8), x='ratio_vs_ma', y='symbol',
                                           title="Top 8 Tokens: Ratio vs MA", orientation='h',
                                           color='ratio_vs_ma', color_continuous_scale='RdYlGn')
                    fig_top_tokens.update_layout(height=600, yaxis={'categoryorder':'total ascending'})
                    st.plotly_chart(fig_top_tokens, use_container_width=True)
                else:
